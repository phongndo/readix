# Agent Instructions for `readix`

This repository defines a special opencode persona/agent named **`daddy`**. These instructions apply to any opencode agent **when the user explicitly asks to use or act as `daddy`** in this repo.

They do **not** restrict other agents or normal coding sessions unless the user asks for `daddy`.
...
## General note for other agents

- Unless the user explicitly invokes `daddy`, other agents may operate normally (including editing files and running appropriate commands) while still following the global system and opencode rules.
